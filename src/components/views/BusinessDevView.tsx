      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div className="kpi-card flex items-center gap-4">
            ...
            <div>
              <p className="text-2xl font-bold">{bdTeam?.length || 0}</p>
              <p className="text-sm text-muted-foreground">BD Team Members</p>
            </div>
          </div>
          <div className="kpi-card flex items-center gap-4">
            ...
            <div key={bd.id} className="kpi-card">
              ...
              <div className="grid grid-cols-2 gap-2 text-sm">
                <div className="text-center p-2 bg-muted/30 rounded">
                  <p className="font-bold">{bdProspects.filter(p => p.bd_owner_id === bd.id).length}</p>
                  <p className="text-xs text-muted-foreground">Leads</p>
                </div>
                <div className="text-center p-2 bg-muted/30 rounded">
                  <p className="font-bold">{bdProspects.filter(p => p.bd_owner_id === bd.id && p.probability >= 60).length}</p>
